...
export const useZebraScanner = ({ onScan, isActive }: ZebraScannerProps) => {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // If the user is typing manually, it's usually >50ms.
            // However, we reset buffer on 'Enter' regardless.

            if (e.key === 'Enter') {
                if (buffer.length > 3) { // Minimum length for a valid tag
                ...
                // If gap is too long, assume it's a new scan or manual typing starting
                if (timeDiff > 100) {
                    buffer = '';
                }
                buffer += e.key;
